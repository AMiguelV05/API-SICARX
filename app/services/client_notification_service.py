...
async def notify_password_reset_requested(client: ClientAccount, token: Optional[str], has_password: bool) -> None:
    ...
    try:
        ...
        if response.status_code not in (200, 201, 202):
            logger.error(f"Frontend rechazo la notificacion de recuperacion de password para {client.email}: {response.status_code} - {response.text}")
            return
        logger.info(f"Notificacion de recuperacion de password enviada al frontend para {client.email}.")
    except Exception as e:
        logger.error(f"Error inesperado notificando recuperacion de password para {client.email}: {type(e).__name__}: {e!r}")
