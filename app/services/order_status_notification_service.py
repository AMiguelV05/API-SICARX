...
async def _notify(order: Order, event: str, status_message: str) -> None:
    ...
    try:
        ...
        await send_signed_webhook(url, raw_body, headers, f"Notificacion '{event}' para la orden {order.uuid}")
    except Exception as e:
        logger.error(f"Error inesperado preparando la notificacion '{event}' sobre la orden {order.uuid}: {type(e).__name__}: {e!r}")
