...
class OrderResponse(CamelModel):
    ...
    serieFolio: Optional[str] = Field(default=None, deprecated=True, description="Ya no aplica - Sicar X no crea ningun documento al hacer checkout. Siempre null.")
    date: Optional[float] = Field(default=None, deprecated=True, description="Ya no aplica - Sicar X no crea ningun documento al hacer checkout. Siempre null.")
    ...
    # deprecated=True (Pydantic, no solo el texto de description): aparece marcado en
    # /openapi.json y renderizado tachado en /docs, asi que un futuro cliente que integre
    # contra esta API ve la desaprobacion en la propia especificacion, no solo si lee el
    # texto libre de la descripcion - mismo criterio para cada [DEPRECATED] de este archivo.
    amount: float = Field(deprecated=True, description="[DEPRECATED, usar `total`] Alias historico de `total` - mismo valor, se conserva por compatibilidad. Nuevas integraciones deben leer `total`.")
...
class OrderCancelResponse(CamelModel):
    documentUuid: str = Field(deprecated=True, description="[DEPRECATED, usar `orderId`] Alias historico - el nombre viene de cuando esto era un UUID de documento real de Sicar X; hoy simplemente hace eco del `order_id` (id o orderUuid) que recibió esta llamada. Se conserva por compatibilidad.")
...
class OrderPublic(CamelModel):
    uuid: str
    sicar_order_id: str = Field(deprecated=True, description="[DEPRECATED, usar `id`] Alias historico - el nombre viene de cuando este id provenia de Sicar X (ya no es el caso, ver CLAUDE.md). Se conserva por compatibilidad.")
