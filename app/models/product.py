...
class Product(Base):
    ...
    __table_args__ = (
        ...
        # Sobre search_normalize(sku)/search_normalize(name), no la columna cruda - busqueda insensible a acentos (f1a3c7e9b2d4) y que ignora guiones (b8d2f5a1c7e3).
        ...
        Index("ix_products_sku_trgm_search", text("search_normalize(sku) gin_trgm_ops"), postgresql_using="gin"),
        Index("ix_products_name_trgm_search", text("search_normalize(name) gin_trgm_ops"), postgresql_using="gin"),
